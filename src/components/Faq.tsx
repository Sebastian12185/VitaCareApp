    ...
    <section id="faq-section" className="scroll-mt-20 px-6 py-16 bg-white">
      ...
      <div className="flex flex-col gap-3.5 max-w-xl mx-auto">
        {faqItems.map((item, idx) => {
          ...
          return (
            <div key={idx} className="border border-slate-200/60 rounded-2xl overflow-hidden transition-all bg-slate-50">
              <button
                onClick={() => setOpenIndex(isOpen ? null : idx)}
                className="w-full flex justify-between items-center p-5 text-left font-bold text-brand-dark hover:text-brand-blue transition-colors text-sm cursor-pointer"
              >
                <span>{item.question}</span>
                <span className={`text-slate-400 transition-transform duration-300 ${isOpen ? 'rotate-180 text-brand-blue' : ''}`}>
                  <span className="sr-only">{isOpen ? 'Zwiń' : 'Rozwiń'}</span>
                  ↓
                </span>
              </button>
